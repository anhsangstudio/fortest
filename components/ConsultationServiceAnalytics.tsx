      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-semibold text-gray-800">Màn hình nhanh cho chủ studio</h2>
            </div>
            <p className="mt-2 text-sm leading-6 text-gray-600">
              Khối này rút gọn toàn bộ báo cáo thành vài điểm chính để xem nhanh trên điện thoại hoặc trong cuộc họp ngắn.
            </p>
          </div>
          <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
            Tập trung vào 3 điều: cần xử lý ngay, điểm sáng cần giữ, xu hướng cần chú ý.
          </div>
        </div>

        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3">
          {ownerQuickSummaryCards.map((item) => (
            <div key={item.tieu_de} className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
              <div className="text-sm font-semibold text-gray-800">{item.tieu_de}</div>
              <div className="mt-2 text-sm leading-6 text-gray-700">{item.noi_dung}</div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3">
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
            <div className="text-sm font-semibold text-emerald-800">3 kết luận quan trọng</div>
            <div className="mt-3 space-y-3">
              {ownerTopTakeaways.length > 0 ? (
                ownerTopTakeaways.map((item, index) => (
                  <div key={`${item.tieu_de}-${index}`} className="text-sm leading-6 text-emerald-900">
                    <strong>{index + 1}. {item.tieu_de}:</strong> {item.noi_dung}
                  </div>
                ))
              ) : (
                <div className="text-sm text-emerald-900">Chưa đủ dữ liệu để rút ra kết luận nổi bật.</div>
              )}
            </div>
          </div>

          <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
            <div className="text-sm font-semibold text-blue-800">3 việc nên làm ngay</div>
            <div className="mt-3 space-y-3">
              {ownerTopActions.length > 0 ? (
                ownerTopActions.map((item, index) => (
                  <div key={`${item.tieu_de}-${index}`} className="text-sm leading-6 text-blue-900">
                    <strong>{index + 1}. {item.tieu_de}:</strong> {item.noi_dung}
                  </div>
                ))
              ) : (
                <div className="text-sm text-blue-900">Chưa có hành động ưu tiên rõ ràng.</div>
              )}
            </div>
          </div>

          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <div className="text-sm font-semibold text-amber-800">3 rủi ro cần để mắt</div>
            <div className="mt-3 space-y-3">
              {ownerTopRisks.length > 0 ? (
                ownerTopRisks.map((item, index) => (
                  <div key={`${item.tieu_de}-${index}`} className="text-sm leading-6 text-amber-900">
                    <strong>{index + 1}. {item.tieu_de}:</strong> {item.noi_dung}
                  </div>
                ))
              ) : (
                <div className="text-sm text-amber-900">Chưa thấy rủi ro nổi bật trong dữ liệu hiện tại.</div>
              )}
            </div>
          </div>
        </div>
