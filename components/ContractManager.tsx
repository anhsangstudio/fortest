...
const handleOpenEdit = async (contract: Contract) => {
  ...
  const filteredServices = services.filter((s: any) => {
    ...
    const tenDv = (s.ten_dv || s.name || '').toLowerCase();
    const maDv = (s.ma_dv || s.code || '').toLowerCase();
    const loaiDv = (s.loai_dv || s.type || '').toLowerCase();
    const chiTiet = (s.chi_tiet_dv || s.description || '').toLowerCase();

    return (
      tenDv.includes(keyword) ||
      maDv.includes(keyword) ||
      loaiDv.includes(keyword) ||
      chiTiet.includes(keyword)
    );
  });
          ...
          >
            ...
            <div className="px-4 md:px-8 py-4 md:py-6 border-b border-slate-100 flex items-center justify-between shrink-0 gap-3">
              ...
              <div className="flex gap-2 shrink-0">
                <button
                  ...
                  className="hidden md:flex items-center gap-2 px-4 py-2 bg-slate-800 text-white rounded-xl hover:bg-slate-900 text-xs font-bold uppercase transition-all shadow-lg active:scale-95"
                >
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 scrollbar-hide">
              <div className="md:hidden">
                <button
                  onClick={handlePreviewPDF}
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-slate-800 text-white rounded-xl hover:bg-slate-900 text-xs font-bold uppercase transition-all shadow-lg active:scale-95"
                >
                  <FileCheck size={16} /> Kiểm tra và xuất PDF
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                  ...
                  <div className="relative self-start sm:self-auto">
                    ...
                    {showServiceDropdown && (
                      <div className="absolute right-0 mt-2 w-72 max-w-[80vw] bg-white border rounded-xl shadow-xl z-50 max-h-60 overflow-y-auto flex flex-col">
              ...
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                  ...
                  <button onClick={handleAddSchedule} className="text-[10px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-200 px-4 py-2 rounded-xl self-start sm:self-auto">+ Thêm mốc thời gian</button>
                </div>
                ...
                <div className="space-y-3">
                  {form.schedules.map((sch) => (
                    <div key={sch.id} className="border rounded-2xl p-4 bg-white shadow-sm">
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 items-start">
                        <div className="md:col-span-3">
                          <label className="text-[9px] font-bold text-slate-500 uppercase">Loại lịch</label>
                          <select className="w-full mt-1 p-2 rounded-lg border border-slate-200 text-sm font-bold" value={sch.type} onChange={e => handleUpdateSchedule(sch.id, { type: e.target.value })}>
                            {scheduleTypes.map(t => <option key={t} value={t}>{t}</option>)}
                          </select>
                        </div>
                        <div className="md:col-span-3">
                          <label className="text-[9px] font-bold text-slate-500 uppercase">Ngày</label>
                          <input type="date" className="w-full mt-1 p-2 rounded-lg border border-slate-200 text-sm font-bold" value={sch.date} onChange={e => handleUpdateSchedule(sch.id, { date: e.target.value })} />
                        </div>
                        <div className="md:col-span-5">
                          <label className="text-[9px] font-bold text-slate-500 uppercase">Ghi chú</label>
                          <input className="w-full mt-1 p-2 rounded-lg border border-slate-200 text-sm" value={sch.notes || ''} onChange={e => handleUpdateSchedule(sch.id, { notes: e.target.value })} />
                        </div>
                        <div className="md:col-span-1 flex md:justify-end">
                          <button onClick={() => handleRemoveSchedule(sch.id)} className="mt-5 text-slate-300 hover:text-red-500 p-2 rounded-lg hover:bg-red-50">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                    <div className="text-[10px] uppercase font-bold text-slate-400 mb-1">Tổng giá trị HĐ</div>
                    <div className="text-xl md:text-2xl font-black text-slate-900">{form.totalAmount.toLocaleString()}đ</div>
                  </div>
                  <div className="bg-emerald-50 rounded-2xl p-4 border border-emerald-100">
                    <div className="text-[10px] uppercase font-bold text-emerald-500 mb-1">Đã thanh toán</div>
                    <div className="text-xl md:text-2xl font-black text-emerald-600">{form.paidAmount.toLocaleString()}đ</div>
                  </div>
                  <div className="bg-red-50 rounded-2xl p-4 border border-red-100">
                    <div className="text-[10px] uppercase font-bold text-red-400 mb-1">Còn lại</div>
                    <div className="text-xl md:text-2xl font-black text-red-500">{(form.totalAmount - form.paidAmount).toLocaleString()}đ</div>
                  </div>
                </div>
