          ...
          <tbody>
            ...
            {!isLoading && paginatedRows.map((row) => {
              ...
              return <tr key={row.id} className="cursor-pointer border-t border-gray-100 hover:bg-gray-50" onClick={() => void openEditModal(row)}>
                <td className="px-3 py-3 text-sm text-gray-700">{row.ngayGuiIn || ''}</td><td className="px-3 py-3 text-sm text-gray-900">{row.tenKhachHang}</td><td className="px-3 py-3 text-sm text-gray-700">{row.contractCode || ''}</td><td className="px-3 py-3 text-sm text-gray-700">{row.tenTrangThai || ''}</td><td className="px-3 py-3 text-sm text-gray-700">{row.tenXuongIn || ''}</td><td className="px-3 py-3 text-sm text-gray-700">{row.nguoiKiemTraNhanAnh || ''}</td><td className="px-3 py-3 text-sm text-gray-700 truncate">{row.linkFiles || ''}</td><td className="px-3 py-3 text-sm text-gray-700">{row.ghiChu || ''}</td>
